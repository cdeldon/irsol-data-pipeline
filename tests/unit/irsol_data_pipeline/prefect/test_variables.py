...
class TestResolveDatasetRoots:
    ...
    def test_falls_back_to_prefect_variable_when_empty(self) -> None:
        with patch(
            "irsol_data_pipeline.prefect.variables.get_variable",
            return_value="/srv/data",
        ):
            result = resolve_dataset_roots("")
    ...
    def test_parametrized_parsing(self, raw: str, expected: list[Path]) -> None:
        result = resolve_dataset_roots(raw)
